...
var searchreplace = (function () {
  ...
  var $_ck4iz3jejhbq25a9 = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_ck4iz3jejhbq25a9.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_dq7hojcjhbq25a2.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_dq7hojcjhbq25a2.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_dq7hojcjhbq25a2.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_dq7hojcjhbq25a2.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_dq7hojcjhbq25a2.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_dq7hojcjhbq25a2.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_dq7hojcjhbq25a2.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_dq7hojcjhbq25a2.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_dq7hojcjhbq25a2.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_dq7hojcjhbq25a2.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_dq7hojcjhbq25a2.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_dq7hojcjhbq25a2.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_dq7hojcjhbq25a2.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_dq7hojcjhbq25a2.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_dq7hojcjhbq25a2.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_60rwirjgjhbq25ag = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_60rwirjgjhbq25ag.open(editor, currentIndexState);
    });
  };
  var $_3v9c3jjfjhbq25af = { register: register };
  ...
  var $_ztb3ijhjhbq25aj = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_3v9c3jjfjhbq25af.register(editor, currentIndexState);
    $_ztb3ijhjhbq25aj.register(editor, currentIndexState);
    return $_blp3g0jbjhbq259z.get(editor, currentIndexState);
  });
  function Plugin () {
  }
