...
var toc = (function () {
  ...
  var $_ax0a1nrkjhbq27w4 = { create: create };

  var tocId = $_ax0a1nrkjhbq27w4.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_1aklyqrjjhbq27w2.getTocClass(editor);
    var headerTag = $_1aklyqrjjhbq27w2.getTocHeader(editor);
    var selector = generateSelector($_1aklyqrjjhbq27w2.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_1aklyqrjjhbq27w2.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_1aklyqrjjhbq27w2.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_49yf9mrfjhbq27vy.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_49yf9mrfjhbq27vy.updateToc(editor);
    });
  };
  var $_iyc6nrejhbq27vx = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_1aklyqrjjhbq27w2.getTocClass(editor);
  ...
  var $_at1p3nrljhbq27w5 = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_49yf9mrfjhbq27vy.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_1aklyqrjjhbq27w2.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_7g6hwbrmjhbq27w6 = { register: register$1 };

  global.add('toc', function (editor) {
    $_iyc6nrejhbq27vx.register(editor);
    $_7g6hwbrmjhbq27w6.register(editor);
    $_at1p3nrljhbq27w5.setup(editor);
  });
  function Plugin () {
  }
